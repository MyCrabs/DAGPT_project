...
def main():
    ...
    st.set_page_config(
        page_title = "👦 Smart Data Analysis Tool", page_icon = "👦", layout = "centered"
    )
    st.header("👦 Smart Data Analysis Tool")
    st.write("Welcome to our Data Analysis Tool. This tool can assist you with some basic task. Pls Enjoy!")
    # LLM Model
    llm = load_llm(model_name=MODEL_NAME)
    logger.info(f"### Successsfully loaded {MODEL_NAME}!###")
    #Upload csv file
    with st.sidebar:
        uploaded_file = st.file_uploader("Upload your file csv here", type="csv")
    # Chat history
    ...
    # Create data analysis agent to query with our data
        ...
        query = st.text_input("Enter yout question:")
